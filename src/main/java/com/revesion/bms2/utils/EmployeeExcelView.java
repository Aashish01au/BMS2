package com.revesion.bms2.utils;
...
public class EmployeeExcelView  extends AbstractXlsxView {
	...
	protected void buildExcelDocument(
		...
		List<EmployeeModel> list = (List<EmployeeModel>) model.get("list");
	...
	private void setBody(Sheet sheet, List<EmployeeModel> list) {
		int rowNum = 1;
		for(EmployeeModel spec : list) {
			Row row = sheet.createRow(rowNum++);
			row.createCell(0).setCellValue(spec.getId());
			row.createCell(1).setCellValue(spec.getFistname());
			...
			row.createCell(5).setCellValue(spec.getSalery());
		}
	}
}
